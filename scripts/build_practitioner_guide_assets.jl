...
const GUIDE_ALGEBRA_TOLERANCE = 1e-10
...
function write_example_results(macros_path, mechanisms_path, summaries)
    ...
    urban.hulten_collapse_error <= GUIDE_ALGEBRA_TOLERANCE ||
        error(
            "urban Hulten-collapse error $(urban.hulten_collapse_error) " *
            "exceeds guide tolerance $GUIDE_ALGEBRA_TOLERANCE",
        )
    open(macros_path, "w") do io
        println(io, "% Generated by scripts/build_practitioner_guide_assets.jl; do not edit.")
        println(io, "\\providecommand{\\GuideUrbanHultenCollapseTolerance}{",
            tex_scientific(GUIDE_ALGEBRA_TOLERANCE; digits=0), "}")
